...
try {
    ...
    $check = 'import runpy,sys; from pathlib import Path; sys.path.insert(0,sys.argv[2]); m=runpy.run_path(sys.argv[1]); s=m["usage_summary"](Path(sys.argv[2])); assert s["status"] == "available" and s["groups"], "No usable usage records"; print("LOCAL SUMMARY VERIFIED: " + str(sum(g["logged_requests"] for g in s["groups"])) + " logged requests")'
    & $python -c $check $staging $jubileeBase
